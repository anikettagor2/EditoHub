                ...
                <div className="p-6 border-b border-border flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 w-full lg:w-auto">
                        <div className="relative w-full sm:w-80">
                             ...
                             <input 
                                type="text" 
                                placeholder={`Global search: overview...`} 
                                className="h-10 w-full rounded-lg border border-border bg-muted/50 pl-11 pr-4 text-xs font-medium text-foreground focus:bg-background focus:border-primary/50 focus:outline-none transition-all placeholder:text-muted-foreground"
                                value={searchQuery}
                        ...
                        <div className="hidden lg:flex items-center gap-2">
                             ...
                             <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Viewing: Overview</span>
                        </div>
                    </div>
                     ...
                     >
                        <thead>
                            <tr className="bg-muted/50 border-b border-border text-left">
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[80px]">Type</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[150px]">Details / Script</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest text-center min-w-[50px]">Done</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[100px]">Assign By</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[120px]">Status</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[120px]">Highlights</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[90px]">Date</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[90px]">Final Date</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[120px]">Editor</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[80px]">Price</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[100px]">Editors</th>
                                <th className="px-3 py-3 text-[9px] font-black text-muted-foreground uppercase tracking-widest min-w-[80px]">Payment</th>
                                <th className="px-3 py-3 border-b border-border w-[50px]"></th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-border">
                            {loading ? (
                                ...
                                projects.filter(p => !searchQuery || p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.clientName?.toLowerCase().includes(searchQuery.toLowerCase())).map((project, idx) => {
                                    ...
                                    >
                                        <td className="px-3 py-4 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                                            {project.videoType || 'N/A'}
                                        </td>
                                        <td className="px-3 py-4">
                                            <div className="flex flex-col gap-1 w-[150px]">
                                                <button 
                                                    onClick={() => { setInspectProject(project); setIsProjectDetailModalOpen(true); }}
                                                    className="text-xs font-black text-foreground tracking-tight leading-tight hover:text-primary transition-colors cursor-pointer text-left focus:outline-none truncate"
                                                    title={project.name}
                                                >
                                                    {project.name}
                                                </button>
                                                <span className="text-[9px] text-muted-foreground font-medium line-clamp-2 leading-tight" title={project.description}>
                                        ...
                                        <td className="px-3 py-4 text-center">
                                            <input 
                                                type="checkbox" 
                                                className="w-3.5 h-3.5 rounded border-border bg-muted cursor-pointer accent-primary"
                                                checked={project.status === 'completed' || project.status === 'approved'}
                                                onChange={(e) => handleUpdateProjectInline(project.id, 'status', e.target.checked ? 'completed' : 'active')}
                                            />
                                        </td>
                                        <td className="px-3 py-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                                        ...
                                        <td className="px-3 py-4">
                                            <select 
                                                ...
                                                className="bg-card text-[9px] font-bold uppercase tracking-widest p-1.5 rounded border border-border outline-none focus:border-primary w-full text-foreground max-w-[110px]"
                                            >
                                        ...
                                        <td className="px-3 py-4">
                                            <input 
                                                ...
                                                className="bg-transparent border-b border-border/50 text-[10px] w-full pb-1 focus:outline-none focus:border-primary transition-colors text-foreground min-w-[100px]"
                                            />
                                        </td>
                                        <td className="px-3 py-4 text-[10px] text-muted-foreground font-bold uppercase tabular-nums">
                                            {new Date(project.createdAt).toLocaleDateString()}
                                        </td>
                                        <td className="px-3 py-4 text-[10px] uppercase font-bold tabular-nums">
                                            {project.deadline ? new Date(project.deadline).toLocaleDateString() : 'TBD'}
                                        </td>
                                        <td className="px-3 py-4">
                                            <select 
                                                value={project.assignedEditorId || ''} 
                                                onChange={async (e) => {
                                                    ...
                                                    if (!newVal) {
                                                        ...
                                                        if (!project.editorPrice) {
                                                            toast.error("Set Editor Price first");
                                                        } else {
                                                            const res = await assignEditor(project.id, newVal, project.editorPrice, project.deadline || '');
                                                            if (res.success) toast.success('Editor assigned!');
                                                            else toast.error(res.error || 'Assignment failed');
                                                        }
                                                    }
                                                }}
                                                className="bg-card text-[9px] font-bold uppercase tracking-widest p-1.5 rounded border border-border outline-none focus:border-primary w-full max-w-[120px] text-foreground"
                                            >
                                                <option value="">Unassigned</option>
                                                {editors.map(ed => <option key={ed.uid} value={ed.uid}>{ed.displayName}</option>)}
                                            </select>
                                        </td>
                                        <td className="px-3 py-4">
                                            <div className="flex items-center gap-1 min-w-[60px]">
                                                ...
                                                <input 
                                                    ...
                                                    placeholder="0"
                                                    className="bg-transparent border-b border-border/50 text-[10px] w-full pb-1 focus:outline-none focus:border-primary transition-colors text-foreground font-black tabular-nums"
                                                />
                                            </div>
                                        </td>
                                        <td className="px-3 py-4 text-[9px] font-bold uppercase tracking-widest text-primary/80">
                                            {editors.find(e => e.uid === project.assignedEditorId)?.displayName || 'None'}
                                        </td>
                                        <td className="px-3 py-4">
                                            <div className="flex flex-col gap-1 justify-center">
                                                <span className={cn("text-[8px] font-black uppercase px-1.5 py-0.5 rounded border w-fit", project.paymentStatus === 'full_paid' ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-red-400/10 text-red-500 border-red-400/20")}>
                                                    CL: {project.paymentStatus === 'full_paid' ? 'Paid' : 'Unpaid'}
                                                </span>
                                                <span className={cn("text-[8px] font-black uppercase px-1.5 py-0.5 rounded border w-fit", project.editorPaid ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-amber-400/10 text-amber-500 border-amber-400/20")}>
                                                    ED: {project.editorPaid ? 'Paid' : 'Pending'}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-3 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <DropdownMenu>
                                                    <DropdownMenuTrigger asChild>
                                                        <button className="h-7 w-7 flex items-center justify-center rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-all active:scale-[0.98]"><MoreHorizontal className="h-3.5 w-3.5" /></button>
                                                    </DropdownMenuTrigger>
