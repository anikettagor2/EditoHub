...
export function ClientDashboard() {
  ...
  const averageProjectValue = projects.length > 0 ? (totalRevenueGenerated / projects.length) : 0;
    ...
    <div className="space-y-10 max-w-[1600px] mx-auto pb-20 pt-4">
       ...
       <div className="flex items-center gap-2 border-b border-border/50 pb-4">
            ...
            <button 
                ...
                className={cn(
                    "px-5 py-2.5 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all flex items-center gap-2",
                    activeTab === 'documents' ? "bg-blue-600 text-white shadow-sm" : "text-muted-foreground hover:bg-muted/50 hover:text-foreground"
                )}
            >
       ...
       {activeTab === 'overview' ? (
        ...
        <>
           ...
           <ClientDocuments userProfile={user} isClient={true} />
       ) : null}
    </div>
  );
}
