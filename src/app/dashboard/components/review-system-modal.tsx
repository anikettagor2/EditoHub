                    ...
                    {(activeTab === 'timeline' ? comments : directConnections).length === 0 ? (
                        ...
                        (activeTab === 'timeline' ? comments : directConnections).map((c) => (
                            <div key={c.id} className="group p-4 rounded-2xl bg-muted/20 border border-border/50 hover:border-primary/20 transition-all space-y-3 relative">
                                ...
                                <p className="text-sm leading-relaxed text-muted-foreground pl-10.5">{c.content}</p>

                                {c.imageUrl && (
                                    <div className="ml-10.5 rounded-xl border border-border/50 overflow-hidden cursor-zoom-in group/img relative">
                                        <img src={c.imageUrl} alt="attachment" className="w-full aspect-video object-cover transition-transform duration-500 group-hover/img:scale-105" />
                                ...
                                {/* Replies Section */}
                                <div className="ml-10.5 space-y-3 pt-3 border-t border-border/30">
                                    {c.replies?.map((r) => (
                                        <div key={r.id} className="space-y-1">
                                            <div className="text-[11px] font-bold text-foreground flex items-center gap-1.5">
                                                {r.userName}
                                                <span className="text-[9px] text-muted-foreground font-medium">{formatDate(r.createdAt)}</span>
                                            </div>
                                    ...
                                    {replyingTo === c.id ? (
                                        <div className="space-y-2 mt-2">
                                            <textarea 
                                                ...
                                                className="w-full text-xs bg-muted/30 border border-border/50 rounded-lg p-2 resize-none focus:outline-none focus:ring-1 focus:ring-primary/30"
                                            />
                                            <div className="flex gap-2">
                                                <button onClick={() => handleAddReply(c.id)} className="flex-1 h-8 rounded-lg bg-primary text-primary-foreground text-[10px] font-bold uppercase hover:brightness-110">Send</button>
                                                <button onClick={() => setReplyingTo(null)} className="h-8 px-4 rounded-lg bg-background border border-border text-[10px] font-bold uppercase hover:bg-muted">Cancel</button>
                                            </div>
                                        </div>
                                    ) : (
                                        <button onClick={() => setReplyingTo(c.id)} className="text-[10px] font-black text-primary/60 hover:text-primary uppercase tracking-wider">Reply</button>
                                    )}
                                </div>
                            </div>
                        ))
