...
export function EditingTimeline() {
    ...
    return (
        <section ref={containerRef} className="relative h-screen bg-[#f7f4ee] overflow-hidden flex flex-col justify-center px-10">
            {/* Ambient Background Lights */}
            <div className="absolute top-0 left-1/4 w-125 h-125 bg-primary/10 blur-[150px] rounded-full" />
            <div className="absolute bottom-0 right-1/4 w-125 h-125 bg-amber-300/15 blur-[150px] rounded-full" />

            <div className="relative z-10 max-w-7xl mx-auto w-full">
                <div className="premium-header mb-2 text-primary">Process 03</div>
                     <h2 className="text-4xl md:text-6xl font-black uppercase text-zinc-900 mb-12 tracking-tighter">
                   The <span className="text-primary italic">Timeline</span> Interface
                </h2>

                {/* Simulated Editor Workspace */}
                <div className="relative bg-white/85 backdrop-blur-2xl border border-black/10 rounded-3xl overflow-hidden shadow-2xl">
                    {/* Toolbar */}
                    <div className="flex items-center gap-4 px-6 py-4 border-b border-black/10 bg-white/90">
                        <Play className="w-4 h-4 text-primary fill-current" />
                        <div className="h-4 w-px bg-zinc-800 mx-2" />
                        <div className="flex gap-4">
                            <Scissors className="floating-tool w-4 h-4 text-zinc-500" />
                            <Layers className="floating-tool w-4 h-4 text-zinc-500" />
                            <Volume2 className="floating-tool w-4 h-4 text-zinc-500" />
                            <Type className="floating-tool w-4 h-4 text-zinc-500" />
                        </div>
                        <div className="ml-auto text-[10px] font-mono text-zinc-500">00:04:12:15</div>
                    </div>

                    {/* Timeline Tracks */}
                    <div ref={timelineRef} className="p-8 space-y-4 relative min-h-75">
                        {/* Playhead */}
                        <div ref={playheadRef} className="absolute top-0 bottom-0 w-0.5 bg-primary z-20 left-0 shadow-[0_0_15px_rgba(99,102,241,0.8)]">
                            <div className="absolute -top-1 -left-1.5 w-4 h-4 bg-primary rotate-45" />
                        </div>

                        {/* Track 1 (Video) */}
                        <div className="h-20 bg-zinc-100 rounded-xl relative overflow-hidden border border-black/10">
                        ...
                        {/* Track 2 (Overlay/GFX) */}
                        <div className="h-12 bg-zinc-100 rounded-xl relative overflow-hidden border border-black/10">
                        ...
                        {/* Track 3 (Audio) */}
                        <div className="h-16 bg-zinc-100 rounded-xl relative overflow-hidden border border-black/10">
                            <div className="absolute left-[0%] w-full h-full px-2 flex items-center gap-1 opacity-20">
